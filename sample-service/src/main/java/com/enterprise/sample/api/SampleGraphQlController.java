...
/**
 * GraphQL counterpart of {@link SampleController}. Demonstrates how a
 * downstream service exposes the same operations over GraphQL using the
 * shared {@link QueryResponse} envelope from {@code common-lib}.
 *
 * <p>Identity is still established by the API gateway and forwarded as
 * {@code X-Auth-Subject} / {@code X-Auth-Roles}; the resolver receives them
 * as schema arguments (mapped from headers in a higher-level filter, or
 * passed by the client during local testing).</p>
 */
@Controller
@Validated
public class SampleGraphQlController {
    ...
    public QueryResponse<WhoAmI> whoAmI(@Argument String subject, @Argument String roles) {
        return QueryResponse.ok(new WhoAmI(
    ...
    @MutationMapping
    public QueryResponse<Order> createOrder(@Argument @Valid CreateOrderInput input,
        ...
        return QueryResponse.ok(new Order(

 ...
 * <ul>
 *   <li>{@code whoAmI} returns the {@code QueryResponse}-shaped envelope.</li>
 *   <li>Validation failure produces an error with stable {@code errorCode}
 *       extension via the platform's {@code GraphQlExceptionResolver}.</li>
 * </ul>
 */
...
class SampleGraphQlControllerTest {
    ...
    void whoAmI_returns_envelope() {
        tester.document("""
                ...
                """)
                .execute()
                .path("whoAmI.status").entity(String.class).isEqualTo("success")
                .path("whoAmI.data.subject").entity(String.class).isEqualTo("alice")
                .path("whoAmI.data.roles").entity(String.class).isEqualTo("[ROLE_USER]");
    }
    ...
    /**
     * Minimal {@code MessageService} stub so the GraphQL exception resolver
     * can resolve i18n keys inside the slice test.
     */
    @TestConfiguration
    static class SchemaSource {
        ...
        com.enterprise.common.i18n.MessageService messageService() {
            ...
            org.mockito.Mockito.when(mock.getOrDefault(

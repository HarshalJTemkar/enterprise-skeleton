...
/**
 * Helper for running a batch of independent tasks <em>concurrently</em> and waiting on the
 * aggregate result. Built on top of the {@code enterpriseParallelExecutor} bean registered by
...
public class ParallelExecutor {
  ...
  /**
   * Constructs a new {@code ParallelExecutor} bound to the shared parallel executor bean.
   *
  ...
  /**
   * Applies {@code mapper} to every item in {@code inputs} concurrently and returns the results in
   * the <em>same order</em> as the input.
   *
   * @param inputs input collection (may be {@code null} or empty)
   * @param mapper conversion function; exceptions propagate via the returned {@link
  ...
  /**
   * Like {@link #map(Collection, Function)} but with a hard timeout for the whole batch. Tasks that
   * have not finished are cancelled.
   *
   * @throws java.util.concurrent.CompletionException wrapping a {@link TimeoutException} if the
   *     aggregate exceeds {@code timeoutMs}
   */
  ...
  /**
   * Launches every {@link Runnable} concurrently and blocks until they all complete. Useful for
   * "fire N writes in parallel" type workflows.
   */
  ...
  /**
   * Expose the backing executor for callers that want to build their own {@link CompletableFuture}
   * pipelines against the shared pool.
   */
  ...
  /**
   * Gracefully shuts down the underlying executor if it is an {@link ExecutorService} (called by
   * the Spring context on destroy).
   */

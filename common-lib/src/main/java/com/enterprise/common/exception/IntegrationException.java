...
/**
 * Thrown when calling an external system (another micro-service, a
 * third-party API, a DB) fails. Maps to HTTP 502 / 503 / 504 depending on
 * the {@link ErrorCode} used.
 */
public class IntegrationException extends BaseException {
    ...
    public static IntegrationException timeout(String target) {
        return new IntegrationException(ErrorCode.INTEGRATION_TIMEOUT,
                "Timed out calling " + target, null);
    }
    ...
    public static IntegrationException unavailable(String target, Throwable cause) {
        return new IntegrationException(ErrorCode.INTEGRATION_UNAVAILABLE,
                target + " is unavailable", cause);
    }
}

...
/**
 * Programmatically builds one {@code "default"} instance per Resilience4j
 * policy, seeded from {@link ResilienceProperties}. Services can either:
 *
 * <ul>
 *   <li>Annotate methods with {@code @CircuitBreaker(name="default")},
 *       {@code @RateLimiter(name="default")}, {@code @TimeLimiter(name="default")},
 *       {@code @Retry(name="default")}, {@code @Bulkhead(name="default")}; or</li>
 *   <li>Inject the registry directly to create ad-hoc named instances.</li>
 * </ul>
 *
 * <p>Every sub-feature can be disabled independently, e.g.:</p>
 * <pre>
...
@ConditionalOnProperty(prefix = "enterprise.common.resilience", name = "enabled",
...
public class ResilienceAutoConfiguration {
    ...
    /**
     * Builds a {@link CircuitBreakerRegistry} containing a single
     * {@code "default"} circuit breaker. The resilience4j-spring-boot3 starter
     * usually provides one itself from YAML, so this bean is only created when
     * the application has not already declared one.
     */
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "enterprise.common.resilience.circuit-breaker",
    ...
    /**
     * Builds a {@link RateLimiterRegistry} with a single {@code "default"}
     * rate limiter: {@code limitForPeriod} permits issued every
     * {@code limitRefreshPeriod}, waiting up to {@code timeoutDuration}.
     */
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "enterprise.common.resilience.rate-limiter",
            name = "enabled", havingValue = "true", matchIfMissing = true)
    ...
    /**
     * Builds a {@link TimeLimiterRegistry} with a single {@code "default"}
     * time limiter that wraps {@link java.util.concurrent.CompletionStage}
     * calls with a timeout.
     */
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "enterprise.common.resilience.time-limiter",
            name = "enabled", havingValue = "true", matchIfMissing = true)
    ...
    /**
     * Builds a {@link RetryRegistry} with exponential backoff based on
     * {@link ResilienceProperties.Retry}.
     */
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "enterprise.common.resilience.retry",
            name = "enabled", havingValue = "true", matchIfMissing = true)
    public RetryRegistry retryRegistry(ResilienceProperties props) {
        var p = props.getRetry();
        RetryConfig.Builder<Object> b = RetryConfig.<Object>custom()
        ...
        if (p.getExponentialBackoffMultiplier() > 1.0d) {
            b.intervalFunction(io.github.resilience4j.core.IntervalFunction
                    .ofExponentialBackoff(p.getWaitDuration().toMillis(),
                            p.getExponentialBackoffMultiplier()));
        } else {
    ...
    /**
     * Builds a {@link BulkheadRegistry} backed by a semaphore bulkhead to cap
     * concurrent callers through a protected section of code.
     */
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(prefix = "enterprise.common.resilience.bulkhead",
            name = "enabled", havingValue = "true", matchIfMissing = true)

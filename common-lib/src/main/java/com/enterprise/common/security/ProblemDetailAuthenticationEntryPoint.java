...
/**
 * Bridges Spring Security's authentication / authorization failures into
 * the same RFC 7807 {@link ProblemDetail} body used by the rest of the
 * platform.
 *
 * <p>Spring Security intercepts {@link AuthenticationException} (401) and
 * {@link AccessDeniedException} (403) <em>before</em> the
 * {@code @RestControllerAdvice} ever runs, so we have to render the response
 * here too.</p>
 */
public class ProblemDetailAuthenticationEntryPoint
    ...
    private void write(HttpServletResponse response, HttpServletRequest request,
        ...
        pd.setProperty("error", new ErrorResponse(
                code.getCode(), localized, correlationId, Instant.now(),
                request.getRequestURI(), List.of(), Map.of()));

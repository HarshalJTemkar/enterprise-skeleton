...
/**
 * Defends downstream microservices against requests that bypass the API gateway. Two complementary
 * checks are supported (either or both):
 *
 * <ol>
 *   <li><b>Shared-secret header</b> — every request must carry {@code X-Gateway-Secret: <secret>}
 *       matching the configured value.
 *   <li><b>Trusted source IP / CIDR list</b> — the remote address must match one of the configured
 *       ranges.
 * </ol>
 *
 * <p>Designed to run very early in the chain (before authentication) so spoofed {@code
 * X-Auth-Subject} / {@code X-Auth-Roles} headers cannot leak into the application.
 *
 * <p>Disabled by default; enable in production via {@code
 * enterprise.common.security.trusted-gateway.enabled=true}.
 */

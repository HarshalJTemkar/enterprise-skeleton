...
/**
 * Ergonomic, null-safe helpers for reading the current {@link Authentication} out of the {@link
 * SecurityContextHolder}.
 *
 * <p>Uses only static methods so it can be called from anywhere (services, JPA auditors, Kafka
 * consumers) without injecting a Spring bean.
 */
public final class SecurityContextHelper {
  ...
  public static Optional<String> currentUsername() {
    return authentication().filter(Authentication::isAuthenticated).map(Authentication::getName);
  }

  /**
   * @return the principal cast to {@link UserDetails} if applicable, otherwise {@link
   *     Optional#empty()}.
   */
  ...
  /**
   * @return {@code true} when the current user has the given role (checks both raw and {@code
   *     ROLE_} prefixed forms).
   */

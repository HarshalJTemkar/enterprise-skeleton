 ...
 * <ul>
 *   <li>JJWT ({@link Jwts}) is on the classpath</li>
 *   <li>{@code enterprise.common.security.enabled=true}</li>
 * </ul>
 *
 * <p>Both the auth-service (issues tokens) and any resource server
 * (validates tokens) consume the same bean — keeping signing keys / TTLs in
 * one place.</p>
 */
@Configuration
@ConditionalOnClass(Jwts.class)
@ConditionalOnProperty(prefix = "enterprise.common.security", name = "enabled",

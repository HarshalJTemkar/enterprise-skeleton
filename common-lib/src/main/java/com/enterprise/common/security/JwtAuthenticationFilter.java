 ...
 *
 * <ol>
 *   <li>Reads the configured {@code Authorization} header (default {@code Authorization: Bearer
 *       …}).
 *   <li>Verifies the signature with {@link JwtTokenProvider} using the same shared secret the
 *       gateway uses.
 *   <li>Builds an {@link Authentication} carrying the subject and the roles claim and stores it in
 *       the {@link SecurityContextHolder}.
 *   <li>Pushes {@code userId} into the SLF4J MDC for log correlation.
 *   <li>If the token is missing/invalid the filter does nothing — Spring Security's {@code
 *       AuthenticationEntryPoint} will reject protected endpoints later.
 * </ol>
 *
 * <p>Activate via {@code enterprise.common.security.resource-server.enabled=true}.
 */
public class JwtAuthenticationFilter extends OncePerRequestFilter {
  ...
  /**
   * Skip the filter for typical public endpoints. Override in a subclass (or via a {@link
   * org.springframework.security.web.SecurityFilterChain}) for finer-grained control.
   */
  @Override

 ...
 *
 * <ul>
 *   <li>method, URI, query, status, content-type
 *   <li>duration in ms
 *   <li>client IP and user-agent
 *   <li>masked request + response bodies (if enabled)
 * </ul>
 *
 * <p>URIs matching {@link LoggingProperties.AccessLog#getExcludePatterns()} are skipped (health
 * checks, Swagger, etc.). Body capture is bounded by {@link
 * LoggingProperties.AccessLog#getMaxPayloadLength()} so we never balloon memory on large uploads.
 */
public class RequestResponseLoggingFilter extends OncePerRequestFilter implements Ordered {
  ...
  protected void doFilterInternal(
    ...
    HttpServletRequest req =
        cfg.isIncludePayload() ? new ContentCachingRequestWrapper(request) : request;
    HttpServletResponse res =
        cfg.isIncludePayload() ? new ContentCachingResponseWrapper(response) : response;
    try {
  ...
  private void writeLog(HttpServletRequest req, HttpServletResponse res, long duration) {
    ...
    if (cfg.isIncludePayload()) {
      if (req instanceof ContentCachingRequestWrapper rw) {
        reqBody =
            masker.mask(truncate(new String(rw.getContentAsByteArray(), StandardCharsets.UTF_8)));
      }

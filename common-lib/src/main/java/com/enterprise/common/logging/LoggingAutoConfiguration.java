...
/**
 * Registers the correlation-id filter, the payload masker and the optional access-log filter. Every
 * filter is independently toggleable:
 *
 * <pre>
...
public class LoggingAutoConfiguration {
  ...
  @ConditionalOnProperty(
      prefix = "enterprise.common.logging.correlation-id",
      name = "enabled",
      havingValue = "true",
      matchIfMissing = true)
  public FilterRegistrationBean<CorrelationIdFilter> correlationIdFilter(LoggingProperties props) {
    FilterRegistrationBean<CorrelationIdFilter> reg =
        new FilterRegistrationBean<>(new CorrelationIdFilter(props.getCorrelationId()));
  ...
  @Bean
  @ConditionalOnProperty(
      prefix = "enterprise.common.logging.access-log",
      name = "enabled",
      havingValue = "true",
      matchIfMissing = true)

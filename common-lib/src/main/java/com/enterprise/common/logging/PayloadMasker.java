...
/**
 * Masks sensitive fields inside JSON / form-encoded payloads before they are written to logs. Uses
 * simple regular expressions — fast and dependency-free.
 *
 * <p>Handles these payload shapes:
 *
 * <ul>
 *   <li>JSON: {@code "password":"abc"} → {@code "password":"***"}
 *   <li>Form / query: {@code password=abc} → {@code password=***}
 *   <li>Authorization header: {@code Bearer xxx} → {@code Bearer ***}
 * </ul>
 */
    ...
    this.formPatterns =
        cfg.getFields().stream()
            .map(
                f ->
                    Pattern.compile(
                        "([?&]?" + Pattern.quote(f) + "=)([^&\\s\"]+)", Pattern.CASE_INSENSITIVE))
            .toList();
  }

  /**
   * Returns {@code payload} with every configured sensitive field replaced by the mask token. Never

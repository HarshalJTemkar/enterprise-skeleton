 ...
 *
 * <p>Two backends are supported and selected with
 * {@code enterprise.common.cache.type}:</p>
 * <ul>
 *   <li>{@code caffeine} (default) – in-process, very fast, per-instance.</li>
 *   <li>{@code redis} – distributed (see nested {@link RedisCache} class).</li>
 * </ul>
 *
 * <p>Disable the whole feature with
 * {@code enterprise.common.cache.enabled=false}.</p>
 */
@Configuration
@EnableCaching
@ConditionalOnProperty(prefix = "enterprise.common.cache", name = "enabled",
...
public class CacheAutoConfiguration {
    ...
    @ConditionalOnProperty(prefix = "enterprise.common.cache", name = "type",
    ...
    /**
     * Redis-backed cache configuration. Loaded as a nested
     * {@link Configuration} so the parent class never references Redis types
     * directly — keeps class-loading clean when Redis is absent.
     */
    @Configuration
    ...
    static class RedisCache {
        ...
        public CacheManager redisCacheManager(
            ...
            var builder = org.springframework.data.redis.cache.RedisCacheManager.builder(cf).cacheDefaults(cfg);
            if (!props.getNames().isEmpty()) {

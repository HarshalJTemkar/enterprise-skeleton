...
/**
 * Populates the {@code /actuator/info} endpoint with build and runtime
 * metadata (Java version, Spring Boot version, active profile, build version
 * and git commit when available).
 *
 * <p>Build metadata is read from properties that can be set at package time:</p>
 * <ul>
 *   <li>{@code build.version}</li>
 *   <li>{@code build.timestamp}</li>
 *   <li>{@code git.commit.id.abbrev}</li>
 * </ul>
 */
@Configuration
@ConditionalOnClass(InfoContributor.class)
@ConditionalOnProperty(prefix = "enterprise.common.info", name = "enabled",

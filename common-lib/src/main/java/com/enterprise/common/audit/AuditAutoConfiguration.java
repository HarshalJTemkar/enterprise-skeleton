...
/**
 * Registers the {@link AuditAspect} and a default SLF4J-based
 * {@link AuditEventPublisher}. Disable entirely with
 * {@code feature.audit.enabled=false}.
 */
@Configuration
@ConditionalOnClass(Aspect.class)
@ConditionalOnProperty(prefix = "feature.audit", name = "enabled",
...
public class AuditAutoConfiguration {
    ...
    public AuditEventPublisher auditEventPublisher() {
        ...
        return event -> audit.info(
                "audit action={} resource={} id={} who={} success={} corrId={} when={} error=\"{}\"",
                event.action(), event.resource(), event.resourceId(),
                event.who(), event.success(), event.correlationId(),
                event.when(), event.errorMessage() == null ? "" : event.errorMessage());
    }

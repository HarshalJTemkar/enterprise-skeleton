...
/**
 * Enables {@code @Scheduled} support and publishes a {@link TaskScheduler}
 * bean fed from {@link SchedulerProperties}.
 *
 * <p>Any service annotated with {@code @Scheduled(cron = "...")} or
 * {@code @Scheduled(fixedDelayString = "${my.job.delay}")} will be picked up
 * automatically. Disable globally with
 * {@code enterprise.common.scheduler.enabled=false}.</p>
 */
@Configuration
@EnableScheduling
@ConditionalOnProperty(prefix = "enterprise.common.scheduler", name = "enabled",
        havingValue = "true", matchIfMissing = true)
@EnableConfigurationProperties(SchedulerProperties.class)
public class SchedulerAutoConfiguration {

    /**
     * Builds the shared {@link TaskScheduler}. A virtual-thread-per-task
     * scheduler is used when {@link SchedulerProperties#isUseVirtualThreads()}
     * is {@code true}, otherwise a bounded {@link ThreadPoolTaskScheduler}.
     *
    ...
    public TaskScheduler taskScheduler(SchedulerProperties props) {
        if (props.isUseVirtualThreads()) {
            ...
            var delegate = Executors.newScheduledThreadPool(1, Thread.ofVirtual()

...
/**
 * Registers a {@link CorsFilter} driven by {@link
 * com.enterprise.common.config.CommonProperties.Web.Cors}.
 *
 * <p>Active only when:
 *
 * <ul>
 *   <li>{@code enterprise.common.web.cors.enabled=true}
 *   <li>The servlet stack is on the classpath
 * </ul>
 *
 * <p>{@code allowedOrigins / allowedMethods / allowedHeaders} are comma-separated lists; pass
 * {@code *} to allow all (the filter automatically switches to {@code allowedOriginPatterns} so
 * credentials still work).
 */
@Configuration
@ConditionalOnClass(CorsFilter.class)
@ConditionalOnProperty(
    prefix = "enterprise.common.web.cors",
    name = "enabled",
    havingValue = "true",
...
public class CorsAutoConfiguration {
  ...
  public CorsFilter corsFilter(CommonProperties props) {
    ...
    cfg.setExposedHeaders(java.util.List.of("X-Correlation-ID", "X-Request-ID", "X-Trace-ID"));

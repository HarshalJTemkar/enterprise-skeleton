 ...
 *
 * <p>Shape mirrors {@link ApiResponse} with an additional {@code pagination}
 * block instead of a free-form {@code data} payload.</p>
 *
...
public record PagedResponse<T>(
    ...
    /**
     * Build a paged response from already-computed values (use this when
     * Spring Data is not involved).
     */
    public static <T> PagedResponse<T> of(List<T> items, int page, int size,
        ...
        return new PagedResponse<>("success", items == null ? List.of() : items,
                p, MDC.get("correlationId"), Instant.now());
    }

    /**
     * Build a paged response from a Spring Data {@code Page}. Kept as an
     * overload using {@code Object} so {@code common-lib} does not force a
     * hard dependency on spring-data-commons at runtime.
     */

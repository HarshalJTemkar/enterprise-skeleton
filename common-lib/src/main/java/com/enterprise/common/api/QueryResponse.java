 ...
 *
 * <p>Mirrors {@link ApiResponse} for REST, but is shaped to coexist with
 * GraphQL's own {@code data} / {@code errors} document. Resolvers should
 * return {@code QueryResponse<T>} as the {@code data} field of the GraphQL
 * type so clients receive a uniform envelope across REST and GraphQL.</p>
 *
 * <p>Example schema:</p>
 * <pre>{@code
 * type WhoAmIResponse {
 ...
 * }</pre>
 *
 * <p>Failures are <em>not</em> wrapped — they are emitted as standard
 * GraphQL errors (with {@code extensions.errorCode} populated by
 * {@code GraphQlExceptionResolver}).</p>
 */
...
public record QueryResponse<T>(
    ...
    public static <T> QueryResponse<T> okWithWarnings(T data, List<String> warnings) {
        return new QueryResponse<>("success", data, null,
                MDC.get("correlationId"), Instant.now(),
                warnings == null ? List.of() : warnings, Map.of());
    }

...
public record CommonProperties(Web web, Security security, Jwt jwt) {

  public CommonProperties {
    if (web == null)
      web = new Web(new Web.ExceptionHandler(true), new Web.Cors(false, "*", "*", "*"));
    if (security == null) security = new Security(false);
    if (jwt == null)
      jwt =
          new Jwt(
              "change-me-change-me-change-me-change-me-1234",
              Duration.ofHours(1),
              Duration.ofDays(7),
              "enterprise-platform",
              "Authorization",
  ...
  public record Web(ExceptionHandler exceptionHandler, Cors cors) {
    ...
    public record Cors(
        boolean enabled, String allowedOrigins, String allowedMethods, String allowedHeaders) {}
  }

...
/**
 * Wires GraphQL features for any service that adds
 * {@code spring-boot-starter-graphql}:
 *
 * <ul>
 *   <li>Custom scalars: {@code DateTime}, {@code Date}, {@code Time},
 *       {@code UUID}, {@code JSON}, {@code Long}, {@code BigDecimal},
 *       {@code Url}, {@code Object}.</li>
 *   <li>{@code MaxQueryDepthInstrumentation} and
 *       {@code MaxQueryComplexityInstrumentation} (configurable thresholds).</li>
 *   <li>{@link GraphQlExceptionResolver} so platform exceptions emit
 *       structured GraphQL errors with the same {@code ErrorCode} as REST.</li>
 * </ul>
 *
 * <p>Disabled with {@code enterprise.common.graphql.enabled=false}. Skipped
 * automatically if Spring GraphQL is not on the classpath.</p>
 */
@AutoConfiguration
...
@ConditionalOnProperty(prefix = "enterprise.common.graphql",
...
public class GraphQlAutoConfiguration {
    ...
    @Bean
    @ConditionalOnProperty(prefix = "enterprise.common.graphql.scalars",
            name = "enabled", havingValue = "true", matchIfMissing = true)
    ...
    public Instrumentation enterpriseMaxQueryComplexity(GraphQlProperties props) {
        int complexity = props.getMaxQueryComplexity() <= 0 ? Integer.MAX_VALUE : props.getMaxQueryComplexity();
    ...
    @Bean
    @ConditionalOnProperty(prefix = "enterprise.common.graphql.error-mapping",
            name = "enabled", havingValue = "true", matchIfMissing = true)

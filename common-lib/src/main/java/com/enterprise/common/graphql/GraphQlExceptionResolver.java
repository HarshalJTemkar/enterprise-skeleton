...
/**
 * Translates platform exceptions into GraphQL errors that carry the same {@link ErrorCode} catalog
 * used by REST. Every error appears under {@code errors[].extensions} as:
 *
 * <pre>{@code
 * {
 *   "extensions": {
 ...
 * }</pre>
 *
 * <p>Mirrors {@code GlobalExceptionHandler} on the REST side so a client sees the same code
...
public class GraphQlExceptionResolver extends DataFetcherExceptionResolverAdapter {
  ...
  private Mapping map(Throwable ex) {
    ...
    if (ex instanceof ConstraintViolationException cv) {
      List<ValidationException.FieldViolation> errs =
          cv.getConstraintViolations().stream()
              .map(
                  v ->
      ...
      return new Mapping(
          ...
          msg(ErrorCode.VALIDATION_FAILED),
          Map.of(),
          errs);
    }
    // Security
    if (ex instanceof BadCredentialsException)
      return basic(ErrorCode.INVALID_CREDENTIALS, ErrorType.UNAUTHORIZED);
    if (ex instanceof LockedException)
      return basic(ErrorCode.ACCOUNT_LOCKED, ErrorType.UNAUTHORIZED);
    if (ex instanceof DisabledException)
      return basic(ErrorCode.ACCOUNT_DISABLED, ErrorType.FORBIDDEN);
    if (ex instanceof AccessDeniedException)
      return basic(ErrorCode.ACCESS_DENIED, ErrorType.FORBIDDEN);
    if (ex instanceof AuthenticationException)
      return basic(ErrorCode.UNAUTHORIZED, ErrorType.UNAUTHORIZED);
    // JWT
    if (ex instanceof ExpiredJwtException)
      return basic(ErrorCode.TOKEN_EXPIRED, ErrorType.UNAUTHORIZED);
    if (ex instanceof JwtException) return basic(ErrorCode.TOKEN_INVALID, ErrorType.UNAUTHORIZED);
    // Persistence
    if (ex instanceof DuplicateKeyException)
      return basic(ErrorCode.DUPLICATE_RESOURCE, ErrorType.BAD_REQUEST);
    if (ex instanceof DataIntegrityViolationException)
      return basic(ErrorCode.DATA_INTEGRITY_VIOLATION, ErrorType.BAD_REQUEST);
    if (ex instanceof OptimisticLockingFailureException)
      return basic(ErrorCode.OPTIMISTIC_LOCK, ErrorType.BAD_REQUEST);
    if (ex instanceof CannotAcquireLockException)
      return basic(ErrorCode.RESOURCE_LOCKED, ErrorType.INTERNAL_ERROR);
    if (ex instanceof EmptyResultDataAccessException)
      return basic(ErrorCode.NOT_FOUND, ErrorType.NOT_FOUND);
    if (ex instanceof QueryTimeoutException)
      return basic(ErrorCode.DATABASE_TIMEOUT, ErrorType.INTERNAL_ERROR);
    if (ex instanceof DataAccessResourceFailureException)
      return basic(ErrorCode.DATABASE_UNAVAILABLE, ErrorType.INTERNAL_ERROR);
    // JVM
    if (ex instanceof IllegalArgumentException)
      return basic(ErrorCode.BAD_REQUEST, ErrorType.BAD_REQUEST);
    if (ex instanceof UnsupportedOperationException)
      return basic(ErrorCode.NOT_IMPLEMENTED, ErrorType.INTERNAL_ERROR);
  ...
  private static ErrorType classify(ErrorCode code) {
    return switch (code) {
      case VALIDATION_FAILED,
              ...
              PRECONDITION_FAILED ->
          ErrorType.BAD_REQUEST;
      case UNAUTHORIZED, INVALID_CREDENTIALS, TOKEN_EXPIRED, TOKEN_INVALID, ACCOUNT_LOCKED ->
          ErrorType.UNAUTHORIZED;
      case FORBIDDEN, ACCESS_DENIED, ACCOUNT_DISABLED, ACCOUNT_EXPIRED, CREDENTIALS_EXPIRED ->
          ErrorType.FORBIDDEN;
  ...
  private GraphQLError toGraphQlError(Mapping m, DataFetchingEnvironment env) {
    ...
    String clientMessage = props.getErrorMapping().isExposeMessage() ? m.message : m.code.name();

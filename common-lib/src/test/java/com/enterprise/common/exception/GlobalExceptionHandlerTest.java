...
class GlobalExceptionHandlerTest {
  ...
  void setUp() {
    ...
    Mockito.when(
            ms.getOrDefault(Mockito.anyString(), Mockito.anyString(), Mockito.any(Object[].class)))
  ...
  void method_not_allowed_returns_405() {
    var r =
        handler.handleMethodNotAllowed(new HttpRequestMethodNotSupportedException("DELETE"), req);
  ...
  void unsupported_media_type_returns_415() {
    var r =
        handler.handleUnsupportedMediaType(new HttpMediaTypeNotSupportedException("text/xml"), req);
  ...
  void not_acceptable_returns_406() {
    var r =
        handler.handleNotAcceptable(new HttpMediaTypeNotAcceptableException("not acceptable"), req);
  ...
  void optimistic_lock_returns_409() {
    var r = handler.handleOptimisticLock(new OptimisticLockingFailureException("stale"), req);
    assertThat(r.getStatusCode()).isEqualTo(HttpStatus.CONFLICT);

...
/**
 * Tracks failed-login attempts per username and locks the account for {@code auth.lockout.duration}
 * after {@code auth.lockout.max-attempts} consecutive failures.
 */
@Service
            ...
            u -> {
              if (u.getLockedUntil() != null && u.getLockedUntil().isAfter(Instant.now())) {
                throw new BusinessException(
                    ErrorCode.ACCOUNT_LOCKED, "Account temporarily locked. Try again later.");
              }
            });
  }

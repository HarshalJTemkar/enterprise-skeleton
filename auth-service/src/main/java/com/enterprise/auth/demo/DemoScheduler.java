...
/**
 * Demonstrates {@code @Scheduled} jobs running on the shared {@code taskScheduler} registered by
 * {@code SchedulerAutoConfiguration}.
 *
 * <p>Each job is independently configurable and can be disabled by setting {@code
 * demo.scheduler.enabled=false} without restarting the scheduler infrastructure.
 */
@Component
@ConditionalOnProperty(
    prefix = "demo.scheduler",
    name = "enabled",
    havingValue = "true",
    matchIfMissing = true)
public class DemoScheduler {
  ...
  /**
   * Fixed-delay job. The delay is externalized so it can be tuned per environment without a
   * rebuild.
   */
  @Scheduled(
      fixedDelayString = "${demo.scheduler.fixed-delay:PT30S}",

...
/**
 * Illustrates how every resilience / async feature from {@code common-lib}
 * can be composed on a single service class. Every method is annotated with
 * Javadoc and a resilience behavior so the integration is obvious.
 */
@Service
public class DemoResilienceService {
    ...
    /**
     * Synchronous call protected by a {@code CircuitBreaker}. When the
     * downstream fails more than
     * {@code enterprise.common.resilience.circuit-breaker.failure-rate-threshold}%
     * of the time, the breaker opens and {@link #unstableFallback(Throwable)}
     * is invoked instead.
     */
    ...
    /**
     * Rate-limited endpoint. The shared {@code "default"} rate limiter allows
     * {@code enterprise.common.resilience.rate-limiter.limit-for-period}
     * calls per refresh period.
     */
    ...
    /**
     * Retries up to {@code max-attempts} times on failure with exponential
     * backoff. Useful for idempotent downstream calls.
     */
    ...
    /**
     * Demonstrates {@code TimeLimiter + CircuitBreaker + @Async} composition.
     * The call returns a {@link CompletableFuture} so Resilience4j's
     * TimeLimiter can cancel it if it exceeds
     * {@code enterprise.common.resilience.time-limiter.timeout-duration}.
     */
    ...
    /**
     * Semaphore-bounded section that caps concurrent callers to
     * {@code enterprise.common.resilience.bulkhead.max-concurrent-calls}.
     */

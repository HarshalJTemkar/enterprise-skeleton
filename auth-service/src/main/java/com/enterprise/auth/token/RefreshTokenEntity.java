...
/**
 * Persistent refresh-token record. Only the SHA-256 hash of the raw token is stored, so a database
 * leak never exposes a usable refresh token.
 *
 * <p>Lifecycle:
 *
 * <ol>
 *   <li>Created on successful {@code /login}.
 *   <li>On {@code /refresh}: marked {@code revoked=true} and a new token is issued (rotation).
 *       {@code replacedBy} points at the new token's hash.
 *   <li>{@code /logout} revokes either the supplied token or all tokens for the principal.
 * </ol>
 */
@Entity
@Table(
...
public class RefreshTokenEntity extends BaseEntity {
  ...
  public void setReplacedBy(String v) {
    this.replacedBy = v;
  }
